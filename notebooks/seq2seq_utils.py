...
class Seq2Seq_Inference(object):
    ...
    def evaluate_model(self, holdout_bodies, holdout_titles, max_len_title):
        """
        ...
        for i in tqdm_notebook(range(num_examples)):
            _, yhat = self.generate_issue_title(holdout_bodies[i], max_len_title)

            self.actual.append(self.pp_title.process_text([holdout_titles[i]])[0])
            self.predicted.append(self.pp_title.process_text([yhat])[0])
